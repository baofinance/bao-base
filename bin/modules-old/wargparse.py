...
logging.basicConfig(
    ...
    handlers=[
        logging.StreamHandler(
            os.fdopen(int(os.environ.get("LOGGING_FILE_DESCRIPTOR")) or 1, "w")
        )
    ],
)
...
class StoreWithOriginAction(argparse._StoreAction):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        logging.debug(
            f"StoreWithOriginAction.__call__({self.dest}, {values}, {option_string})"
        )
...
class StoreConstWithOriginAction(argparse._StoreConstAction):
    def __init__(
        ...
        **kwargs,
        ...
        logging.debug(
            f"StoreConstWithOrigin.__init__({option_strings}, {dest}, {const}, {default}"
        )
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        logging.debug(
            f"StoreConstWithOriginAction.__call__(...{values}, {option_string})"
        )
...
class StoreTrueWithOriginAction(StoreConstWithOriginAction):
    ...
    def __init__(
        self, option_strings, dest, default=False, required=False, help=None, **kwargs
    ):
...
class StoreFalseWithOriginAction(StoreConstWithOriginAction):
    ...
    def __init__(
        self, option_strings, dest, default=True, required=False, help=None, **kwargs
    ):
...
class CountWithOriginAction(argparse._CountAction):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        logging.debug(
            f"CountWithOriginAction.__call__({self.dest},{values},{option_string})..."
        )
        ...
        setattr(
            ...
            self.dest,
            {
                ...
                "origin": (
                    ...
                    else f"{current_origin} {option_string}"
                ),
            },
        )
...
class RichArgumentParser:
    def __init__(self, spec):
        ...
        self.parser.register(
            "action", "boolean_optional", BooleanOptionalWithOriginAction
        )
        ...
        for arg_spec in spec_obj.get("arguments", []):
            ...
            self.parser.add_argument(
                *arg_spec["names"], **arg_spec
            )  # leave names in for default values
...
def main():
    ...
    if sys.argv[1] in ["-h", "--help"]:
        ...
        parser = RichArgumentParser(
            sys.argv[1] if len(sys.argv) and len(sys.argv[1]) > 1 else "{}"
        )

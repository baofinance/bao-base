...
def toNamedDataFrame(input_data: str) -> tuple[pd.DataFrame, str]:
    ...
    header_match = re.search(r"^\| Contract\s+\|.+\|$", input_data, re.MULTILINE).group(
        0
    )
    if not header_match:
    ...
    rows_match = re.findall(
        r"^\|\s*[A-Za-z0-9$_]+\s*\|\s*[0-9]+.+\|$", input_data, re.MULTILINE
    )
    if not rows_match:
    ...
    df = pd.DataFrame(data, columns=header).drop(
        columns=["Initcode Size (B)", "Runtime Margin (B)", "Initcode Margin (B)"]
    )

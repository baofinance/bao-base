...
def toNamedDataFrame(input_data: str) -> tuple[pd.DataFrame, str]:
    ...
    df = pd.DataFrame(data, columns=header).drop(columns=["Initcode Size (B)", "Initcode Margin (B)"])
    df["Runtime Size (B)"] = df["Runtime Size (B)"].apply(lambda x: x.replace(",", ""))
    df["Runtime Size (B)"] = pd.to_numeric(df["Runtime Size (B)"])

    df["Runtime Margin (B)"] = df["Runtime Margin (B)"].apply(lambda x: x.replace(",", ""))
    df["Runtime Margin (B)"] = pd.to_numeric(df["Runtime Margin (B)"])

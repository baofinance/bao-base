...
def toNamedDataFrame(input_data: str) -> tuple[pd.DataFrame, str]:
    ...
    if not path_match:
        raise ValueError(
            "Input data does not contain the expected 'file:contract' pattern."
        )
    ...
    header_match = re.search(
        r"^\| Function Name\s+\|.+\|$", input_data, re.MULTILINE
    ).group(0)
    if not header_match:
        raise ValueError("Input data does not contain a valid table header.")
    header = [
        col.strip().lower() for col in re.split(r"\s+\|\s+", header_match.strip("| "))
    ]

    # Extract rows containing 'src/' and clean them
    rows_match = re.findall(
        r"^\| [A-Za-z$_][A-Za-z$_]+\s+\|.+\|$", input_data, re.MULTILINE
    )
    if not rows_match:

...
@test "_output function should output" {
  ...
  expect --not --contains "bats"
  expect --contains "This should be visible"
}
...
@test "debug function should output when script name is in DEBUG" {
  ...
  expect --contains "DEBUG"
  expect --contains "Test debug message"
}
...
@test "info function should respect verbosity level" {
  ...
  expect --contains "Should be visible with verbosity 1"

  run info 2 "Should not be visible with verbosity 1"
  expect --contains --not "Should not be visible with verbosity 1"

  # Test with higher verbosity
  export BAO_BASE_VERBOSITY=2

  run info 1 "Should be visible with verbosity 2"
  expect --contains "Should be visible with verbosity 2"

  run info 2 "Should also be visible with verbosity 2"
  expect --not --contains "bats"
  expect --contains "Should also be visible with verbosity 2"
...
@test "sensitive function should hide text in output" {
  ...
  expect --contains "This contains ***hidden*** which should be hidden"

  # Run with custom replacement
  run bash -c "source './bin/run/logging' &&
                sensitive 'SECRET_VALUE' 'not secret text' &&
                _output 'TEST' 'This contains SECRET_VALUE which should be hidden'"
  expect --contains "This contains not secret text which should be hidden"
  ...
  expect --contains "This contains SECRET_VALUE which should be shown"
...
Third line"
  ...
  expect --head --contains "TEST"
  expect --head --contains "First line"
  expect --tail --contains "Third line"

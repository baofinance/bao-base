...
- Three ownership patterns for UUPS contracts:
  - **BaoOwnable** (legacy): `_initializeOwner(finalOwner)` uses `msg.sender` as temp owner. Deploy via `_deployProxyViaStubAndRecord` (needs UUPSProxyDeployStub so msg.sender = FactoryDeployer, not BaoFactory).
  - **HarborOwnable** (modern): `_initializeOwner(deployerOwner, pendingOwner)` takes explicit deployer. Deploy via `_deployProxyAndRecord` (direct, no stub).
